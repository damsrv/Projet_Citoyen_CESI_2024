        ...
        <main className="flex flex-col w-full">
            <header className="flex items-center gap-2 p-3 border-b">
                <Avatar>
                    ...
                    <AvatarFallback>{interlocutor.firstname.charAt(0).toUpperCase()}{interlocutor.lastname.charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
                <p>{interlocutor.firstname} {interlocutor.lastname}</p>
            </header>
            <div className="flex grow overflow-auto">
                <ul className="flex grow flex-col px-4 py-2 gap-3 justify-end h-full">
            ...
            <div className="flex gap-2 p-2">
                <Input className="flex grow" type="text" value={message} onChange={(e) => setMessage(e.target.value)}/>
                <Button type="button" disabled={loading} onClick={() => postMessage(message)}>Envoyer</Button>
            </div>
        </main>
    )
}
